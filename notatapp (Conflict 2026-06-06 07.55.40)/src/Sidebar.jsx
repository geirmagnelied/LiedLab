...
import { ChevronDown, ChevronRight, FolderOpen, Trash2, AlertCircle,
         FileText, Plus, Star } from 'lucide-react'
    ...
    return (
      <div>
        <div className="proj-row"
          ...
          {/* Favorite star */}
          <button
            onClick={e => { e.stopPropagation(); onToggleFavorite && onToggleFavorite(p.id) }}
            className="fav-btn"
            title={p.favorite ? 'Fjern favoritt' : 'Marker som favoritt'}
            style={{ background:'none', border:'none', cursor:'pointer', padding:'1px 2px',
              display:'flex', flexShrink:0, opacity: p.favorite ? 1 : 0,
              transition:'opacity .15s' }}>
            <Star size={11}
              fill={p.favorite ? 'rgba(255,220,50,.9)' : 'none'}
              ...
              : pNotes.map(n => {
                  ...
                  return (
                    <div key={n.id} className="note-link"
                      ...
                      <FileText size={10} color="rgba(255,255,255,.35)" style={{ flexShrink:0 }}/>
    ...
    <div className="sidebar-brand" style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:10, color:'rgba(255,255,255,.45)', fontWeight:500 }}>
              {projects.length} prosjekt · {notes.filter(n=>!n.done).length} aktive
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex:1, overflowY:'auto', borderTop:'1px solid rgba(255,255,255,.1)', paddingTop:4 }}>
        ...
        <button onClick={() => setProjsOpen(v=>!v)}
          ...
          Alle prosjekt
          <span style={{ marginLeft:'auto', fontSize:10, opacity:.5 }}>{projects.length}</span>
        </button>
